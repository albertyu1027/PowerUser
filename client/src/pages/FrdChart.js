...
class Chart extends Component {
    ...
    addFriend = () => {
        ...
        API
            ...
            .then(res => {
                ...
                // Each time a user is added the bar-chart color is removed from the array. The
                // user can not add any more friends when the array reaches a length of zero
                if (shards_clr.length === 0) {
                    alert("You have reached the max number of friends");
                    return;
                }

                // Setup a temporary new friend obect. This is pushed to the React state at the
                ...
                // Reset the leaderboard each time the API call runs. This prevents user's from
                // appearing in the leaderboard multiple times
    ...
    sortUsers() {
        ...
        // Store the state.leaderboard and state.sortedFriends arrays in variables. This
        // is just to keep the code clean.
        var leaderboard = this.state.leaderboard;
        var sortedFriends = this.state.sortedFriends;
        // Empty the friends tabel each time sortUsers is run (prevents duplication of
        // the table rows)
        var friendTable = "";
        // Clear out the sorted array, also prevents any duplication
        sortedFriends = [];

        // Stored the leaderboard object into an array, so we can sort the users by
        // their cost/values
      ...
      <td>$${friend[1]}</td>
        ...
        document
            .getElementById("leaderboard_list")
            .innerHTML = friendTable;

    }

    componentDidMount() {
        // Reset the colors array each time teh component loads (incase the load is not
        // a page refresh, but navigating from page to page)
        shards_clr = ["#007bff", "#c4183c", "#17c671", "#00b8d8"];

        // Get the user's data from MongoDB
        API
            .getUpload(this.props.user._id)
            .then(res => {
                // Initialize 12months of rows for each user. This is incase they have less than
                // 3 months of data
                var costArr = [
                    0,
                    0,
                ...
                // Store the API response (in this case user's uploads) in the user_data
                // variable
                ...
                <div className="row">
                    ...
                    <div className="col-md-4">
                        ...
                        <table className="table">
                            ...
                            <tbody id="leaderboard_list"></tbody>
                        </table>
                    </div>
                </div>
            </Container>
